...
function Results(props) {
  ...
  return(
    <div className={`${styles.ResultsHolder} ${props.currentPos === 0 ? styles.Center : ""} ${props.currentPos === -1 ? styles.Left : ""}`}>
      
      <div style={{display: "flex", alignItems: "center", flexDirection: "column", gap: "5px"}}>
        <p className={styles.Title}>Results</p>
        <p>Here’s what ChatGPT had to say!</p>
      </div>

      <Result text={props.responseText}/>

      <div className={`${styles.Submit} ${styles.Selectable}`} onClick={props.finished}> Start Over </div>
